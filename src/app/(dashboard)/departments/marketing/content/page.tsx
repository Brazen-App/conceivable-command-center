...
function toContentItems(news: NewsItem[], research: ResearchItem[], reddit: RedditPost[]): ContentItem[] {
...
export default function MarketingContentPage() {
  ...
  const fetchData = useCallback(async (initial = false) => {
    ...
    try {
      ...
      setItems(toContentItems(data.newsItems ?? [], data.researchItems ?? [], data.redditPosts ?? []));
    } catch { /* */ }
  ...
  const postCard = async (itemId: string, cardId: string) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        const isQueued = cardResult?.queued;
        setState(itemId, (s) => ({ ...s, cards: s.cards.map((c) => c.id === cardId ? { ...c, status: isQueued ? "queued" as const : "posted" as const, posting: false, postError: isQueued ? "queued" : null } : c) }));
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => fetchData(false)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border" style={{ borderColor: "var(--border)", color: "var(--foreground)", backgroundColor: "var(--surface)" }}>
            ...
            <div className="p-4">
              <div className="flex items-start gap-3">
                <div className="px-2 py-1 rounded text-[10px] font-bold uppercase shrink-0 mt-0.5" style={{
                  backgroundColor: item.type === "news" ? "#356FB614" : item.type === "research" ? "#9686B914" : "#F1C02814",
                  color: item.type === "news" ? "#356FB6" : item.type === "research" ? "#9686B9" : "#F1C028",
                }}>{item.type}</div>
                    ...
                    <div className="flex gap-2 mb-2">
                      ...
                      >
                        {state.heroImageLoading ? (
                          ...
                          <><ImageIcon size={12} /> {state.heroImage ? "Regenerate" : "Generate Image"}</>
                        )}
                      </button>
                    </div>
...
function PlatformCard({
  card, topic, onPost, onQueue, onUpdateBody, onRegenerate, onFeedback, onApproveImage,
}: {
  card: DraftCard;
  topic: string;
  onPost: () => void;
      ...
      <div className="rounded-xl border overflow-hidden flex flex-col" style={{ borderColor: card.imageApproved ? `${GREEN}60` : "var(--border)", backgroundColor: "var(--surface)" }}>
        ...
        <div className="relative h-44 flex items-center justify-center cursor-pointer" style={{ background: card.imageUrl ? undefined : meta.gradient }} onClick={() => card.imageUrl && setImageExpanded(true)}>
          {card.imageUrl ? (
            ...
            <div className="flex flex-col items-center gap-1 cursor-pointer" onClick={(e) => { e.stopPropagation(); onRegenerate(); }}>
              <ImageIcon size={18} className="text-white/50" />
              <span className="text-white/40 text-[9px] uppercase tracking-wider">Click to generate</span>
            </div>
          )}
